...
<!-- ---------------------------Side-Bar Close------------------------------- -->
							...
							<div class="GridLex-col-9_sm-8_xs-12">
								...
								<div class="admin-content-wrapper">
                                    ...
                                    $stmt = $conn->prepare("SELECT * FROM tbl_academic_qualification WHERE member_no = '$id' ORDER BY id LIMIT $page1,5");
                                    $stmt->execute();
									...
									<a href="app/drop-academic.php?id=<?php echo $row['id']; ?> & ref=<?php echo "$id"?>" onclick = "return confirm('Are you sure you want to delete this qualification ?')" class="btn btn-primary btn-sm btn-inverse">Delete</a>
									...
									<form action="app/update-academic-qualification.php?ref=<?php echo "$id"?>" method="POST" autocomplete="off" enctype="multipart/form-data">
                                ...
                                $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                                $stmt = $conn->prepare("SELECT * FROM tbl_academic_qualification WHERE member_no = '$id' ORDER BY id");
                                $stmt->execute();
                                $result = $stmt->fetchAll();
								...
								?>
				                    ...
				                    <div class="modal-body">
									<form action="app/add-academic-qualification.php?ref=<?php echo "$id"?>" method="POST" autocomplete="off" enctype="multipart/form-data">
					                <div class="row gap-20">
									<div class="col-sm-12 col-md-12">
