...
?>
  				<div class="admin-section-title">
                                    ...
                                    $stmt = $conn->prepare("SELECT * FROM tbl_training WHERE member_no = '$id' ORDER BY id LIMIT $page1,5");
                                    $stmt->execute();
									...
									<a href="app/drop-training.php?id=<?php echo $row['id']; ?> & ref=<?php echo "$id"?>" onclick = "return confirm('Are you sure you want to delete this training ?')" class="btn btn-primary btn-sm btn-inverse">Delete</a>
									...
									<form action="app/update-training.php?ref=<?php echo "$id"?>" method="POST" autocomplete="off" enctype="multipart/form-data">
                                ...
                                $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                                $stmt = $conn->prepare("SELECT * FROM tbl_training WHERE member_no = '$id' ORDER BY id");
                                $stmt->execute();
                                $result = $stmt->fetchAll();
								...
								?>
				                    ...
				                    <div class="modal-body">
									<form action="app/add-training.php?ref=<?php echo "$id"?>" method="POST" autocomplete="off" enctype="multipart/form-data">
					                <div class="row gap-20">
