...
try {
	...
	if ($records > 0) {
        ...
        window.location.href='../register.php?p=$role';
        </script>
        ";
...
try {
    ...
    $emailverr = substr($email, strrpos($email, '@' )+7);
                if(strcmp($emailverr, 'uiu.ac.bd') == 0){
    ...
    window.location.href='../register.php?p=Employee';
    </script>
    ";
	// header("location:../register.php?p=Employee&r=1123");
            }else{echo" 
            <script>
            alert('Invalid University Mail ID')
            window.location.href='../register.php?p=Employee';
            </script>
            ";
        }
...
try {
	...
	header("location:../register.php?p=Employer");				  
	}catch(PDOException $e)
    {
